...
- **`/etc/init.d/step-ca` (the OpenRC service script) is genuinely untracked, by contrast.** Checked the *entire* `scripts.tar` for step-related entries - there's only the one `.pre-install` script above, nothing that writes an init script. The live file is dated May 2025 with empty shell history around it - it predates whatever tracking exists now and isn't reproduced by anything in this repo. **This means a `step-ca` container rebuild would fail today**: `configure_container`'s `rc-service step-ca start` assumes this file already exists. Needs to become a proper Terraform-managed `file{}` push (same pattern as `smb.conf`) before any rebuild is attempted - this is a prerequisite independent of the apk-vs-curl question below.
