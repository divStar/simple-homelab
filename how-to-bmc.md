...
- **IP Address**: `192.168.60.53`
- **Subnet Mask**: `255.255.255.0`
- **Default Gateway**: `192.168.60.1` — required, the firmware rejects `0.0.0.0` outright
...
- Static IP in the same subnet, e.g. `192.168.60.244`
- Subnet mask `255.255.255.0`
- **Router/Gateway field: must be set** (e.g. `192.168.60.1`) — leaving it blank leaves the OS's
...
- **That subnet must not overlap with any other currently-active interface** (Wi-Fi, etc.) — an
  ...
  disjoint `192.168.60.0/24` fixed it.
- Browse to `https://192.168.60.53` — self-signed cert, click through the warning.
...
ipmitool lan set 1 ipaddr 192.168.60.53
ipmitool lan set 1 netmask 255.255.255.0
ipmitool lan set 1 defgw ipaddr 192.168.60.1
